...
<script>
	import { getRandomNum_AB, GL } from '@/utils/activity.js'
	import { delayTime, formatDateTime } from '@/utils/common.js'
				...
				this.$apis.user_updbalance({
					...
					uni.showModal({
						...
						success: (res) => {
							...
							if (isOuhuang) {
								//重置用户的体验属性
								this.$apis.user_recoverexperience().then(async res => {
									await delayTime(1000)
...
<style lang="less" scoped>
	.lottery-wrap {
		height: 100%;
		background: url(http://guoshao-service.test.upcdn.net/file-path/images/cao-bg.jpg);
	}
