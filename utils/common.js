...
// 重新渲染设备数据
export const reloadPhone = async () => {
...
// 判定设备是否损坏
export function isDamage(_this, id) {
	showModal('该设备出现故障，是否选择回收设备,然后重新购买一台？', true, (e) => {
		if (e.confirm) {
			_this.$apis.gamep_update(id, {
				...
				_this.$apis.gamep_resetphone(id).then(res => {
					if (res.code == 200) {
						// 重置状态
						_this.$apis.user_recoverexperience().then(res => {
							setInterval(() => {
...
let timer
// 判定设备是否到期 --实现方法
async function expirtimes(_this, id, expirtime, e) {
...
export const setVerik = async (_this) => {
	// const wxVeri = await getWxveri(_this) || ""
	// wxVeri == "666" || wxVeri == "yes"
	uni.setStorageSync("isveriDate", true)
}
